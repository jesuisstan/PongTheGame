		...
		const res = {
			winner: {
				...
				position:
					...
					this.game_state.player1.profile.user.id
						? 1
						: 2,
			},
			loser: {
  ...
  private _check_ball_collide_paddle(
		...
		if (ball.collidable) {
			...
			if (this._check_colide(ballColide, paddleFrontUpCollideZone)) {
				...
				this._check_colide(ballColide, paddleFrontMiddleCollideZone)
			) {
				ball.direction.x *= -1;
				ball.velocity += Default_params.BALL_SPEED_INCREASE;
				this._disable_collision(ball);
				...
				this._check_colide(ballColide, paddleFrontDownCollideZone)
			) {
				ball.direction.x *= -1;
				ball.direction.y += Default_params.BALL_PERTURBATOR;
				ball.velocity += Default_params.BALL_SPEED_INCREASE;
